    ...
    <div className="space-y-3">
      {rooms.map((r, index) => (
        <AnimatedWrapper key={r.id} animation="scale" delay={index * 100}>
          <Link to={`/rooms/${r.id}`} className="block p-3 rounded-lg border-2 border-gray-100 hover-scale transition-all">
            <div className="flex items-center justify-between">
              <div>
                <div className="font-medium">Chat with {r.members.filter(m=>m.user).map(m=>m.user.name).join(', ')}</div>
                <div className="text-xs text-gray-500">Tier: {r.tier}</div>
              </div>
